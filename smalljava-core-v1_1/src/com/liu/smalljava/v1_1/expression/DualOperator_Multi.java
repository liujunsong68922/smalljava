...
public class DualOperator_Multi extends DualAbstractOperator {


@Override
public boolean evaloperator() {
	// 执行减法操作
	return this.eval_threechild_multi();
}
 ...
 * @return
 */
private boolean eval_threechild_multi() {
	OperElementData leftoper = astnode.children.get(0).operdataresult;
	if (leftoper == null) {
		System.out.println("eval_threechild_multi.程序逻辑错误，左操作对象为null");
		return false;
	}
	if (leftoper.getElementdatatype() == null) {
		System.out.println("eval_threechild_multi.程序逻辑错误，左操作对象类型为null");
		return false;
	}
	if (leftoper.getElementdatatype().equals("int")) {
		...
		intoper.doMulti(astnode.children.get(2).getOperdataresult().toString());
	...
	if (leftoper.getElementdatatype().equals("long")) {
		...
		longoper.doMulti(astnode.children.get(2).getOperdataresult().toString());
	...
	if (leftoper.getElementdatatype().equals("float")) {
		...
		floatoper.doMulti(astnode.children.get(2).getOperdataresult().toString());
	...
	if (leftoper.getElementdatatype().equals("double")) {
		...
		doubleoper.doMulti(astnode.children.get(2).getOperdataresult().toString());
		astnode.operdataresult = doubleoper;
		return true;
	}
	System.out.println("【ERROR】乘号操作遇到了不支持的数据类型：" + leftoper.getElementdatatype());
	return false;
}
}