...
public class DualOperator_Devide extends DualAbstractOperator {

	@Override
	public boolean evaloperator() {
		// 执行减法操作
		return this.eval_threechild_devide();
	}
	 ...
	 * @return
	 */
	private boolean eval_threechild_devide() {
		OperElementData leftoper = astnode.children.get(0).operdataresult;
		if (leftoper == null) {
			System.out.println("eval_threechild_devide.程序逻辑错误，左操作对象为null");
			return false;
		}
		if (leftoper.getElementdatatype() == null) {
			System.out.println("eval_threechild_devide.程序逻辑错误，左操作对象类型为null");
			return false;
		}
		if (leftoper.getElementdatatype().equals("int")) {
			...
			intoper.doDevide(astnode.children.get(2).getOperdataresult().toString());
		...
		if (leftoper.getElementdatatype().equals("long")) {
			...
			longoper.doDevide(astnode.children.get(2).getOperdataresult().toString());
		...
		if (leftoper.getElementdatatype().equals("float")) {
			...
			floatoper.doDevide(astnode.children.get(2).getOperdataresult().toString());
		...
		if (leftoper.getElementdatatype().equals("double")) {
			...
			doubleoper.doDevide(astnode.children.get(2).getOperdataresult().toString());
