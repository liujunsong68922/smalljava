...
public class DualOperator_LogicLittleEqual extends DualAbstractOperator{

	@Override
	public boolean evaloperator() {
		// <=
		return eval_threechild_litterequal();
	}
	 ...
	 * @return
	 */
	private boolean eval_threechild_litterequal() {
		OperElementData leftoper = astnode.children.get(0).operdataresult;
		if (leftoper == null) {
			System.out.println("eval_threechild_litterequal.程序逻辑错误，左操作对象为null");
			return false;
		}
		if (leftoper.getElementdatatype() == null) {
			System.out.println("eval_threechild_litterequal.程序逻辑错误，左操作对象类型为null");
			return false;
		}
		if (leftoper.getElementdatatype().equals("int")) {
			...
			boolean bresult = intoper.doLE(astnode.children.get(2).getOperdataresult().toString());
		...
		if (leftoper.getElementdatatype().equals("long")) {
			...
			boolean bresult = longoper.doLE(astnode.children.get(2).getOperdataresult().toString());
			BooleanOperElement booloper = new BooleanOperElement(bresult);
			astnode.operdataresult = booloper;
			return true;
		}
		if (leftoper.getElementdatatype().equals("float")) {
			...
			boolean bresult = floatoper.doLE(astnode.children.get(2).getOperdataresult().toString());
			BooleanOperElement booloper = new BooleanOperElement(bresult);
			astnode.operdataresult = booloper;
			return true;
		}
		if (leftoper.getElementdatatype().equals("double")) {
			...
			boolean bresult = doubleoper.doLE(astnode.children.get(2).getOperdataresult().toString());
			BooleanOperElement booloper = new BooleanOperElement(bresult);
			astnode.operdataresult = booloper;
			return true;
		}

		System.out.println("【ERROR】小于等于号操作遇到了不支持的数据类型：" + leftoper.getElementdatatype());
		return false;
	}	
}
